...
def make_batch(data, batch_size, Nmax):
    ...
    if use_cuda:
        batch = torch.tensor(torch.from_numpy(np.stack(strokes,1)).cuda().float())
    else:
        batch = torch.tensor(torch.from_numpy(np.stack(strokes,1)).float())
...
class Model():
    ...
    def kullback_leibler_loss(self):
        ...
        if use_cuda:
            KL_min = torch.tensor(torch.Tensor([hp.KL_min]).cuda()).detach()
        else:
            KL_min = torch.tensor(torch.Tensor([hp.KL_min])).detach()
    ...
    def conditional_generation(self, epoch):
        ...
        if use_cuda:
            sos = torch.tensor(torch.Tensor([0,0,1,0,0]).view(1,1,-1).cuda())
        else:
            sos = torch.tensor(torch.Tensor([0,0,1,0,0]).view(1,1,-1))
        s = sos
        seq_x = []
        seq_y = []
        seq_z = []
